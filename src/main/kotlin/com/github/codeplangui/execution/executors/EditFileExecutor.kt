 ...
 * In trust mode, applies change directly with inline highlighting.
 */
class EditFileExecutor(
    private val fileChangeReview: FileChangeReview,
    private val inlineChangeHighlighter: InlineChangeHighlighter
) : ToolExecutor {
    ...
    override suspend fun execute(input: JsonObject, context: ToolContext): ToolResult {
        ...
        return withContext(Dispatchers.IO) {
            ...
            val isTrusted = fileChangeReview.sessionFileWriteTrusted
            val vf = LocalFileSystem.getInstance().findFileByIoFile(file)

            // In trust mode: snapshot for highlighter before write
            if (isTrusted && vf != null) {
                inlineChangeHighlighter.beforeFileChanged(vf)
            }
            ...
            // Apply inline highlights in trust mode
            if (isTrusted) {
                val vfAfter = LocalFileSystem.getInstance().refreshAndFindFileByIoFile(file)
                if (vfAfter != null) {
                    inlineChangeHighlighter.onFileChanged(vfAfter)
                }
            }
