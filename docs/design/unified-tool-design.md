...
> **原则**：Agent 的思考/推理过程应作为日志输出，**不应在聊天流中每一步输出一个气泡**。

| 维度 | 要求 | 原因 |
|------|------|------|
| 思考过程 | 归入 `tool_step_start` / `tool_step_end` 事件中的 inline 步骤卡片，不单独弹气泡 | 每步思考一个气泡导致信息碎片化，用户需要反复滚动，体验差 |
| 执行进度 | 通过 `ExecutionCard` 组件展示，支持折叠/展开 | 保持聊天流整洁，用户按需查看详情 |
| 结果输出 | 工具执行完成后输出**单个**汇总气泡 | 多个气泡增加认知负担，一个完整的输出更易理解 |

**How to apply**：所有 Agent Loop 中的中间状态（如"正在分析文件"、"找到了 3 处匹配"）通过 `log` 事件或 `tool_step_*` 事件传递，前端在对应的 `ExecutionCard` 内展示，不创建独立的聊天消息。
